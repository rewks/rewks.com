...
Now go back into the user you just created and go to the <Keyword text="Security Credentials"/> tab. Scroll down to Access keys and click the Create access key button. Choose "Command Line Interface" and continue.
...
Go to the VPC dashboard and click on <Keyword text="Create VPC"/>. In the menu select the <Keyword text="VPC and more"/> option and go through the inputs, choose whatever tag and CIDR blocks you wish, just make sure you select 1 public subnet otherwise an Internet Gateway won't be set up.
