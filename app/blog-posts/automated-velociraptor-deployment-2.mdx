...
3. We're also going to be passing in the VPC and Subnet data sources we grabbed from the Networking module, so we need to declare variables for these.
...
10. Ok so that is the infrastructure set up, but we'll want to think about users and configure access. We don't really want to do everything as root, so we can use a cloud-init script to add a user to the system after first boot and add the public key from the previously created key pair to it for SSH access. First create a new file with the following contents.
